...
async function render() {
    ...
    if (q) {
        const f = s => [s.title, s.starts_at, s.ends_at].join(" ").toLowerCase().includes(q);
        active = active.filter(f); archived = archived.filter(f);
    }
    if (filter) {
        const f = s => slotStatus(s) === filter;
        active = active.filter(f); archived = archived.filter(f);
    }
      ...
      <div class="rounded-2xl p-4 bg-white/85 backdrop-blur border border-gray-200 shadow-sm">
        <div class="flex items-start justify-between gap-3">
          <div>
            ...
            <div class="font-semibold" style="color:#AF9778">${s.title}</div>
          ...
          <div class="flex flex-col gap-2 shrink-0">
            ${s.archived ? `` : `<button type="button" class="text-sm px-3 py-1 rounded-xl" style="background:#AF9778;color:white" onclick="openBooking('${s.id}')">Buchung</button>`}
<button type="button" class="text-sm px-3 py-1 rounded-xl bg-gray-100 hover:bg-gray-200" onclick="editSlot('${s.id}')">Slot bearbeiten</button>
<button type="button" class="text-sm px-3 py-1 rounded-xl ${s.archived ? 'bg-emerald-100 hover:bg-emerald-200' : 'bg-slate-100 hover:bg-slate-200'}"
        onclick="toggleArchive('${s.id}', ${!s.archived})">${s.archived ? 'Aus Archiv holen' : 'Archivieren'}</button>
<button type="button" class="text-sm px-3 py-1 rounded-xl bg-rose-100 hover:bg-rose-200" onclick="deleteSlot('${s.id}')">Slot löschen</button>
    ...
    const collapsedActive = JSON.parse(localStorage.getItem(LS_ACTIVE_COLLAPSED) || "false"); // Standard: offen
    const collapsedArchive = JSON.parse(localStorage.getItem(LS_ARCH_COLLAPSED) || "true");  // Standard: zu
  ...
  `;
    ...
    // Status nur speichern – nicht neu rendern (sonst Endlosschleife möglich)
    if (act) act.addEventListener("toggle", () => {
        const collapsed = !act.open;
        localStorage.setItem(LS_ACTIVE_COLLAPSED, JSON.stringify(collapsed));
        const lbl = act.querySelector("summary span.text-slate-500");
        if (lbl) lbl.textContent = collapsed ? "ausklappen" : "einklappen";
    });
    if (arch) arch.addEventListener("toggle", () => {
        const collapsed = !arch.open;
        localStorage.setItem(LS_ARCH_COLLAPSED, JSON.stringify(collapsed));
        const lbl = arch.querySelector("summary span.text-slate-500");
        if (lbl) lbl.textContent = collapsed ? "ausklappen" : "einklappen";
    });
  ...
  <div class="mt-2 text-sm">
    ...
    <ul class="space-y-1">
      ${list.map(b => {
        ...
        return `
        <li class="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-lg px-2 py-1 cursor-pointer hover:bg-gray-100"
...
function bindCreateSlotHandler() {
    formSlot.onsubmit = (ev) => {
        ...
        const slot = {
            id: uid(),
            title,
            capacity: Number($("#sl_capacity").value || 0),
            starts_at: new Date($("#sl_starts").value).toISOString(),
            ends_at: new Date($("#sl_ends").value).toISOString(),
            archived: false
        };
...
window.toggleArchive = (id, flag) => {
    const slots = load(LS_SLOTS); const s = slots.find(x => x.id === id); if (!s) return;
    s.archived = !!flag; save(LS_SLOTS, slots); render();
};

window.deleteSlot = (id) => {
    if (!confirm("Diesen Termin (Slot) löschen? Alle Buchungen dazu werden mit gelöscht.")) return;
    save(LS_SLOTS, load(LS_SLOTS).filter(s => s.id !== id));
    save(LS_BOOKINGS, load(LS_BOOKINGS).filter(b => b.slotId !== id));
    render();
};
...
formBooking.addEventListener("submit", (ev) => {
    ...
    if (editing) { const i = all.findIndex(x => x.id === old.id); all[i] = booking; }
    else { all.push(booking); }
    save(LS_BOOKINGS, all);

    // Triviar-Hinweis + Copy
...
function openModal(target) {
    [modalBooking, modalSlots].forEach(m => { if (!m) return; m.classList.add("hidden"); m.classList.remove("flex"); });
...
document.querySelectorAll("[data-close='booking']").forEach(b => b.onclick = () => {
    formBooking.reset(); ctx.currentBookingId = null; ctx.currentSlotId = null; closeModal(modalBooking);
});
document.querySelectorAll("[data-close='slots']").forEach(b => b.onclick = () => {
    formSlot.reset(); formSlot.onsubmit = null; closeModal(modalSlots);
});
...
function showToast(msg, href, label = "Öffnen") {
    ...
    if (href) {
        toastLink.href = href;
        toastLink.textContent = label;
        toastLink.onclick = null;
...
function showToastWithAction(msg, label, onClick) {
    toastMsg.textContent = msg;
    toastLink.href = "#";
    toastLink.textContent = label || "OK";
...
function maybeShowBackupReminder() {
    ...
    showToastWithAction("Backup fällig – jetzt sichern.", "Jetzt sichern", () => {
        exportBackup();
